...
export interface AdjacencyMap {
    edgeLists: Map<string, { to: string; weight: number; edgeId: string }[]>
    isWeighted: boolean
    isDirected: boolean
    sizeEnum: string
}
...
export const buildAdjacencyMap = (
    ...
    for (i = 0; i < V; i++) {
        for (var j = 0; j < V; j++) {
            if (i === j)
                // don't want nodes with edges pointing to themselves
                continue
            ...
            if (roll <= EdgeFactor) {
                ...
                adjMap.edgeLists
                    .get(from)!
                    .push({ to: to, weight: weight, edgeId: '' })
                if (!directed) {
                    // if not directed, edge should be two-way
                    adjMap.edgeLists
                        .get(to)!
                        .push({ to: from, weight: weight, edgeId: '' })
                }
            }
        }
    }
 ...
 */
export const generateEdgeAnchors = (
    adjMap: AdjacencyMap,
    pMap: PositionMap
) => {
    ...
    adjMap.edgeLists.forEach(
        (value: { to: string; weight: number }[], key: string) => {
            value.forEach((value: { to: string; weight: number }) => {
                ...
                if (Math.abs(yDiff) <= Math.abs(xDiff)) {
                    // use left and right anchorings of the nodes
                    if (xDiff < 0) {
                        // occurs when the "from" node is left of the "to" node
                        edgeDescriptor = getEdgeDescriptor(
                            value.to,
                            key,
                            Position.Left,
                            Position.Right
                    ...
                    // use top and bottom anchorings of the nodes
                    if (yDiff < 0) {
                        // occurs when the 'from' node is above the 'to' node
                        edgeDescriptor = getEdgeDescriptor(
                            value.to,
                            key,
                            Position.Top,
 ...
 */
export const buildElementList = (
    adjMap: AdjacencyMap,
    posMap: PositionMap,
    anchors: EdgeAnchorMap
) => {
    ...
    for (i = 0; i < numV; i++) {
        ...
        for (var j = 0; j < edgeList.length; j++) {
            ...
            let sourcePosChar = edgeAnchorComponents[0].charAt(
                edgeAnchorComponents[0].length - 1
            )
            let targetPosChar = edgeAnchorComponents[1].charAt(
                edgeAnchorComponents[1].length - 1
            )
